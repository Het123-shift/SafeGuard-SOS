 ...
 * / power-button detector — one code path, three entry points.
 */
class SOSNativeModule(reactContext: ReactApplicationContext) :
    ...
    fun ensureForegroundServiceRunning(promise: Promise) {
        // Call once on app start (and ideally on boot via a BOOT_COMPLETED
        // receiver) so the lock-screen notification + power-button listener
        // are alive before an emergency happens, not started reactively.
